...
static unsigned int gpioLED = 45;       ///< hard coding the LED gpio for this example to P9_23 (GPIO49)
static unsigned int gpioLED2 = 44; 
static unsigned int gpioButton = 30;   ///< hard coding the button gpio for this example to P9_27 (GPIO115)
static unsigned int gpioButton2 = 31; 
...
// gpio_set_value(gpioLED, ledOn);          // Not required as set by line above (here for reference)
   ...
   printk(KERN_INFO "GPIO_TEST2: The button state is currently: %d\n", gpio_get_value(gpioButton2));
...
static irq_handler_t ebbgpio_irq_handler2(unsigned int irq, void *dev_id, struct pt_regs *regs){
   ...
   printk(KERN_INFO "GPIO_TEST2: Interrupt2! (button state is %d)\n", gpio_get_value(gpioButton2));
